...
public class ArcEdgeFactory implements EdgeFactory {
    ...
    @Override
    public Element create(Document doc, SvgPoint p1, SvgPoint p2) {
        final Element result = doc.createElement("path");
        result.setAttribute("fill", "none");
        final SvgPoint c = getCenterWithOffset(p1, p2);
    ...
    private SvgPoint getCenterWithOffset(SvgPoint p1, SvgPoint p2) {
        final double length = SvgUtils.length(p1, p2);
        final double sin = SvgUtils.sin(p1, p2);
        final int ax = (int) (-sin * EDGE_OFFSET_RATIO * length);
        final double cos = SvgUtils.cos(p1, p2);
        final int ay = (int) (cos * EDGE_OFFSET_RATIO * length);
